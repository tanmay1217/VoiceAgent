...
def voice():
    ...
    if not user_input: return jsonify({"user_said": "...", "response": "I didn't catch that.", "audio": None})
    logger.info(f"USER_VOICE: {user_input}")
    response_text = orch.process_text_input(user_input)
    logger.info(f"AGENT_RESP: {response_text}")
    audio = orch.speech_service.text_to_speech(response_text)
    return jsonify({"user_said": user_input, "response": response_text, "audio": audio.hex() if audio else None})
