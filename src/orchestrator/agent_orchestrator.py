...
class AgentOrchestrator:
    ...
    def process_text_input(self, user_input: str) -> str:
        ...
        # ROUTING GUARD: If a booking is active and the user provides date or time,
        # we MUST treat this turn as a booking update, even if intent is misclassified.
        if is_booking_active and (entities.get('date') or entities.get('time')):
            logger.info("Overriding intent to 'booking' due to active session and date/time entities.")
            intent = 'booking'
        ...
        if intent == 'booking' or is_booking_active:
            ...
            for key, value in extracted.items():
                if value and value not in ["null", "None"]:
                    # Do not let user slang overwrite the official vehicle selection
                    if key in ['vehicle_id', 'vehicle_name'] and is_booking_active:
                        continue
                    self.booking_details[key] = value

        # Routing Logic (Prioritize Booking if active)
        if intent == 'greeting' and not is_booking_active:
            response = self._handle_greeting()
        
        # If booking is active, we prioritize handle_booking over inquiries 
        # to prevent listing the whole inventory by mistake.
